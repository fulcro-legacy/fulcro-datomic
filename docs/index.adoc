...
The Fulcro Datomic library includes a few useful additions that make it easier to integrate one or more Datomic
...
Fulcro Datomic includes a database component that can be easily added to an Fulcro Server. The steps
are quite simple:
...
Create a database component is as simple as `(fulcro.datomic.core/build-database :NAME)`. The
...
https://github.com/fulcro-web/fulcro-todomvc/blob/master/src/server/todomvc/system.clj[initial setup:]
...
The schema support in Fulcro Datomic is a fork of https://github.com/Yuppiechef/datomic-schema[Yuppiechef's Datomic Schema] with additions for experimental additional

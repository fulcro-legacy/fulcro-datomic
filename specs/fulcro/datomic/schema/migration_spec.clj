(ns fulcro.datomic.schema.migration-spec
  (:require
    [datomic.api :as d]
    [fulcro.datomic.schema :as schema]
    [fulcro-spec.core :refer [specification assertions when-mocking component behavior provided]]
...
(t/use-fixtures
  :once #(timbre/with-merged-config
          {:ns-blacklist ["fulcro.datomic.schema"]}
          (%)))
...
(specification "all-migrations*"
  (behavior "INTEGRATION - finds migrations that are in the correct package and ignores the template"
    (let [result (schema/all-migrations* "fulcro.datomic.fixtures.migrations")
          migA (first result)
          migB (second result)]
      (assertions
        migA => {:fulcro.datomic.fixtures.migrations/A {:txes [[{:item 1}]]}}
        (keys migB) => [:fulcro.datomic.fixtures.migrations/B]
        (get-in migB [:fulcro.datomic.fixtures.migrations/B :txes]) => [[{:item 2}]]
        (get-in migB [:fulcro.datomic.fixtures.migrations/B :migrate-data]) =fn=> fn?)))
...
(specification "migrate"
  (provided "when provided with data migration functions"
    ...
    (let [migrate-called (atom false)]
      (schema/migrate migrate-called "fulcro.datomic.fixtures.migrations")
      (assertions

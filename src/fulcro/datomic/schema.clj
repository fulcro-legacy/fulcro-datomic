(ns fulcro.datomic.schema
  (:require
    [datomic.function :as df]
    [io.rkn.conformity :as conformity]
    [fulcro.datomic.impl.util :as n]
...
all-migrations
  (if (#{"0" "false"} (System/getenv "FULCRO_DATOMIC_CACHE_MIGRATIONS"))
    all-migrations*
    (do (timbre/warn "Caching migrations, set env var FULCRO_DATOMIC_CACHE_MIGRATIONS to 0 or false to disable.")
...
; `fulcro.datomic.impl.core-schema-definitions` for the code that installs the two transactor functions `ensure-version`
...
(defn vtransact
  ...
  (let [db-ignored           (datomic/db connection)
    ...
    (try
      ...
      (catch Exception e (future (throw e))))))
...
(defdbfn refcas [db eid rel old-value new-value] :db.part/user
  (fulcro.datomic.schema/refcas* db eid rel old-value new-value true))
...
(defdbfn refcas-remove [db eid rel old-value new-value] :db.part/user
  (fulcro.datomic.schema/refcas* db eid rel old-value new-value false))

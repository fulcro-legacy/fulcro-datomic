(ns untangled.datomic.impl.core-schema-definitions
...
(defn ensure-constraints-conform [conn]
  (let [schema (s/generate-schema
                 [
                  ...
                  (s/with-require [[untangled.datomic.impl.validation :as v]]
                                  (s/dbfn constrained-transaction [db transaction] :db.part/user
                                          (let [result (d/with db transaction)]
                                            (untangled.datomic.impl.validation/validate-transaction result false)
                                            transaction
                                            )
                                          )
                                  )
